...
import { TraitBadgeInfo, getTraitBadgeInfo, getTraitExplanation } from '../../utils/traitUtils';
export type { TraitBadgeInfo };
...
export const TabletopCanvas: React.FC<TabletopCanvasProps> = ({
  ...
  const handleSquadGroupMouseDown = (e: React.MouseEvent, unit: Unit) => {
    ...
    const isOwnerTurn = isPvP ? (activePlayer === playerRole && unit.owner === playerRole) : (unit.owner === activePlayer);
    const canDrag = (isOwnerTurn || (activePhase === 'Deployment' && unit.isPendingDeploymentConfirm) || unit.isPendingDisembarkConfirm) && (
  ...
  const handleCanvasDrop = (e: React.DragEvent) => {
    ...
    // Hard Deployment Zone Restriction Check
    if (activePhase === 'Deployment') {
      if (isPvP && (unit.owner !== playerRole || activePlayer !== playerRole)) {
        setRejectedPlacementNotice(`⚠️ You can only deploy your own units on your deployment turn.`);
        setTimeout(() => setRejectedPlacementNotice(null), 3500);
        return;
      }
