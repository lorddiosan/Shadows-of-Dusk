...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className={`w-full sticky top-0 z-50 px-3 py-2 flex items-center justify-between shadow-lg select-none transition-colors duration-300 ${
      ...
      <div className="flex items-center space-x-2.5 cursor-pointer shrink-0" onClick={() => setTab('home')}>
        <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-[#9a281e] to-[#c94a29] flex items-center justify-center shadow-md border border-[#e07b53]/40 shrink-0">
          <span className="text-base">⚔️</span>
        </div>
        <div>
          <div className="flex items-center space-x-2">
            <span className="font-black text-sm tracking-wider text-[#f4efe6] uppercase font-serif">WARPATH</span>
            <span className="text-[10px] px-1.5 py-0.2 rounded bg-[#241710] text-[#d49e54] border border-[#593d28] font-mono hidden xs:inline">Dusk Core</span>
          </div>
        </div>
      </div>

      {/* Main Tabs (Configured: Home Page -> Matchmaking -> Battlefield -> Armies -> Guilds -> ...) */}
      <nav className="flex items-center space-x-1 sm:space-x-1.5 overflow-x-auto scrollbar-none mx-2 shrink">
        <button
          onClick={() => setTab('home')}
          className={`shrink-0 flex items-center space-x-1.5 px-3 py-1.5 rounded-md font-medium text-xs transition-all ${
            currentTab === 'home'
